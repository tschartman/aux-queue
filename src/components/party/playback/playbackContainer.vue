...
export default {
  ...
  methods: {
    ...
    playCurrentSong () {
      spotifyApi.put('/me/player/play').then(res => {
        //  if (res.status === 204) {
        console.log(res)
        this.paused = false
        //  }
    ...
    pauseCurrentSong () {
      spotifyApi.put('/me/player/pause').then(res => {
        if (res.status === 204) {
          this.paused = true
        }
      })
    ...
    skipNext () {
      spotifyApi.post('/me/player/next').then(res => {
        //  if (res.status == 204) {
        console.log(res)
        this.init()
        //  }
    ...
    skipPrevious () {
      spotifyApi.post('/me/player/previous').then(res => {
        //  if (res.status == 204) {
        console.log(res)
        this.init()
        // }
    ...
    init () {
      ...
      spotifyApi.get('/me/player/currently-playing').then(res => {
